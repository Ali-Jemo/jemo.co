...
export default function LivingMuseumTimeline({ showFull = false }: LivingMuseumTimelineProps) {
  const events = showFull ? TIMELINE_EVENTS : TIMELINE_EVENTS.slice(0, 6);
  ...
  return (
    <div className="relative">
      ...
      <div className="relative border-r-2 border-[var(--line)] pr-6 md:pr-10 space-y-12 max-w-4xl mx-auto">
        {events.map((evt, idx) => (
          <motion.div
            key={evt.year}
          ...
          >
            {/* Timeline node icon */}
            <div
              className={`absolute -right-[31px] md:-right-[47px] top-1.5 w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all ${
                evt.highlight
                  ? "bg-[var(--brand)] border-white text-white shadow-lg shadow-[var(--brand)]/30 scale-110"
                  : "bg-[var(--surface)] border-[var(--line)] text-[var(--ink-2)] group-hover:border-[var(--brand)] group-hover:text-[var(--brand)]"
              }`}
            >
              {evt.highlight ? <Sparkles className="w-4 h-4" /> : <CheckCircle2 className="w-4 h-4" />}
            </div>

            <div className="bg-[var(--surface)] border border-[var(--line)] p-6 rounded-2xl group-hover:border-[var(--brand)]/50 transition-all shadow-xs">
              <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                <span className="font-mono text-2xl font-extrabold text-[var(--brand)]">
                  {evt.year}
                </span>
              ...
              <h3 className="text-xl font-bold text-[var(--ink-1)] mb-2 group-hover:text-[var(--brand)] transition-colors">
                {evt.title}
              </h3>
